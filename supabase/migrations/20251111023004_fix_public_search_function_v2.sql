/*
  # إصلاح دالة البحث العام - النسخة 2

  1. الوصف
    - تصحيح مشكلة GROUP BY في استعلام الطرود
    - استخدام subquery للحصول على الطرود قيد التوصيل مع الترتيب الصحيح

  2. التغييرات
    - إعادة كتابة استعلام الطرود لتجنب مشكلة GROUP BY
    - استخدام COALESCE بشكل صحيح للحالات التي لا توجد فيها طرود
*/

-- ===================================
-- إعادة إنشاء دالة البحث العام مع إصلاح مشكلة GROUP BY
-- ===================================
...
BEGIN
  ...
  -- البحث عن الطرود قيد التوصيل (استخدام subquery للتجنب مشكلة GROUP BY)
  SELECT COALESCE(
    (
      SELECT jsonb_agg(package_info)
      FROM (
  ...
  v_result := jsonb_build_object(
    ...
    'in_delivery_package', v_in_delivery_package,
    'has_pin', v_has_pin
  );
...
-- تعليق توضيحي
COMMENT ON FUNCTION public_search_beneficiary IS 'دالة البحث العام الآمن - النسخة المصححة بالكامل';
